...
const userVerifier = async (req, res) => {
  try {
    ...
    return res.status(201).send({ status: true, message: "Congratulations Your have registered and verified successfully", Welcome: "Registred successfully, Thanks to become a part of our family", details: updateProfile })
  } catch (err) {
    res.status(500).send(err.message)
  }
}
